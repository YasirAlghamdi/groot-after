...
public class CutPanelProduct implements Serializable {
	private CutPanelProductProduct cutPanelProductProduct = new CutPanelProductProduct();
	...
	public void initCutOptions() {
		cutPanelProductProduct.initCutOptions(this);
	}
	...
	public void init(CutPanel cutPanel) {
		...
		cutPanelProductProduct.setCutStrings(new ArrayList<String>());
		Object[] keys = cutMap.keySet().toArray();
		Object[] treeCuts = cutMap.values().toArray();
		for (int i = 0; i < cutMap.keySet().size(); i++) {
			cutPanelProductProduct.getCutStrings().add((String) keys[i]);
		}
		...
		cutPanelProductProduct.initCutOptions(this);
		if (cutPanelProductProduct.getCutStrings() != null && cutPanelProductProduct.getCutStrings().size() > 0) {
			c.gridy++;
			leftPanel.add(this.cutPanelProductProduct.getCutOptions(), c);
		}
		c.gridy++;
